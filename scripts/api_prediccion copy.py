...
def recomendaciones(dias: int = 7):
    ...
    # 1) Predicción futura (ya lo tenías)
    try:
        resp = requests.get(f"http://127.0.0.1:8000/prediccion?dias={dias}")
        pred = resp.json()
    ...
    for i, dia in enumerate(pred):
        ...
        # ---------- RESUMEN SEMANAL (versión mejorada) ----------
    ...
    # Tomamos como referencia la última fecha real de la tabla
    ...
    if datos_mes:
        ...
        # Nivel de riesgo mensual + acción prioritaria
        if lluvia_total > 40:
...
def alertas():
    ...
    # Riesgo acumulado (3 días seguidos)
    ...
    # -----------------------------
    try:
        resp = requests.get("http://127.0.0.1:8000/prediccion?dias=7")
        pred = resp.json()
    except:
        pred = []
    ...
    # -----------------------------
    if reales and pred:
        # Estrés térmico ahora + subiendo en predicción
        ...
        # Humedad alta ahora + radiación baja → hongos
        ...
        # Viento moderado ahora + radiación alta → quemaduras
    ...
    # Humedad alta + radiación baja repetida
